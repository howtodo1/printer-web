...
function findWasmBinary() {
  ...
  if (Module['locateFile']) {
    return locateFile('printthing.wasm');
  }

  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL('printthing.wasm', import.meta.url).href;
...
var ASM_CONSTS = {
 ...
 1181844: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 1182832: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 1182915: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 1182984: () => { return window.innerWidth; },  
 1183014: () => { return window.innerHeight; }
};

// Imports from the Wasm binary.
var _flatten = Module['_flatten'] = makeInvalidEarlyAccess('_flatten');
...
function assignWasmExports(wasmExports) {
  assert(typeof wasmExports['flatten'] != 'undefined', 'missing Wasm export: flatten');
  ...
  _flatten = Module['_flatten'] = createExportWrapper('flatten', 4);
